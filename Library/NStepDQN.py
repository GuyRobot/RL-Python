"""
    N-step:
        Q(s,a) = r_s,a + ymax_a'Q(s_t+1,a_t+1)
               = r_t + ymax_a[r_a,t+1 + ymax_a'(Q(s_t+2, a')]
               = r_t + y*r_t+1 + y^2*max_a'(Q(s_t+2, a')

    Almost the same:
     Except:
        exp_source = ptan.experience.ExperienceSourceFirstLast(env,
            agent, gamma=params['gamma'], steps_count=args.n)
        loss_v = common.calc_loss_dqn(batch, net,
