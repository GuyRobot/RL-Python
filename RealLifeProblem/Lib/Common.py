...
class RewardTracker:
    ...
    def reward(self, reward_steps, frame, epsilon=None):
        ...
        speed = (frame - self.t_start_frame) / (time.time() - self.t_start)
        self.t_start_frame = frame
        self.t_start = time.time()
        ...
        print("%d done %d games, mean reward %.3f, mean steps %.2f, speed %.2f f/s %s"
              % (frame, len(self.total_rewards) * self.group_rewards,
                 mean_reward, mean_steps, speed, epsilon))
        sys.stdout.flush()
...
def calc_loss(batch, net, tgt_net, gamma, device="cpu"):
    ...
    next_states_actions = net(next_states_v).max(1)[1]
    next_states_values = tgt_net(next_states_v)\
        .gather(1,next_states_actions.unsqueeze(-1)).squeeze(-1)
    next_states_values[done_mask] = 0.0

    expected_state_action_values = next_states_values.detach() * gamma + rewards_v
