...
Prices = collections.namedtuple('Prices', field_names=['open', 'high', 'low',
                                                       'close', 'volume'])


def read_csv(file_name, sep=',', filter_data=True, fix_open_price=False):
    print("Reading", file_name)
    with open(file_name, 'rt', encoding='urf8') as fd:
        ...
        for row in reader:
            ...
            if fix_open_price and pre_vals is not None:
                ppo, pph, ppl, ppc, ppv = pre_vals
                if abs(po - ppc) > 1e-8:
                    count_fixed += 1
                    po = ppc
                    pl = min(pl, po)
...
def prices_to_relative(prices):
    ...
    return Prices(open=prices.open, high=rh,
                  low=rl, close=rc, volume=prices.volumn)
...
def load_year_data(year, base_dir="data"):
    y = str(year)[-2:]
    result = {}
    for path in glob.glob(os.path.join(base_dir, "*_%s.csv" % y)):
