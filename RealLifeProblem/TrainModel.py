...
import argparse
import numpy as np
from RL.RealLifeProblem.Lib import Common
import torch
import torch.optim as optim

from RL.RealLifeProblem.Lib import DataHandler, Models, Enviroment, Validation
...
if __name__ == '__main__':
    ...
    if args.year is not None or os.path.isfile(args.data):
        if args.year is not None:
            stock_data = DataHandler.load_year_data(args.year)
        else:
            stock_data = {"YNDX": DataHandler.load_relative(args.data)}
        env = Enviroment.StocksEnv(stock_data, bars_count=BARS_COUNT,
                                   reward_on_close=True, state_1d=False,
                                   volumes=False)
        env_test = Enviroment.StocksEnv(stock_data, bars_count=BARS_COUNT,
                                        reset_on_close=True, state_1d=False)
    elif os.path.isdir(args.data):
        env = Enviroment.StocksEnv.from_dir(args.data, bar_count=BARS_COUNT,
                                            reset_on_close=True, state_1d=False)
        env_test = Enviroment.StocksEnv.from_dir(args.data, bar_counts=BARS_COUNT,
                                                 reset_on_close=True, state_1d=False)
    else:
        raise RuntimeError("No data to train")
    env = gym.wrappers.TimeLimit(env, max_episode_steps=1000)

    val_data = {"YNDX": DataHandler.load_relative(args.valdata)}
    env_val = Enviroment.StocksEnv(val_data, bars_count=BARS_COUNT,
                                   reward_on_close=True, state_1d=False)

    writer = SummaryWriter(comment='-simple-' + args.run)
    net = Models.SimpleFFDQN(env.observation_space.shape, env.action_space.n)
    ...
    with Common.RewardTracker(writer, stop_reward=np.inf, group_rewards=100) as reward_tracker:
        while True:
            step_idx += 1
            buffer.populate(1)
            ...
            loss_v = Common.calc_loss(batch, net, tgt_net, GAMMA, device=device)
            loss_v.backward()
            optimizer.step()
            ...
            if step_idx % CHECKPOINT_EVERY_STEP == 0:
                ...
                torch.save(net.state_dict(), os.path.join(saves_path,
                                                          'checkpoint-%3d.data' % idx))
